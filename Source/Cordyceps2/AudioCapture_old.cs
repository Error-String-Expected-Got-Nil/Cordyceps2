...
public class AudioCapture_old : MonoBehaviour
{
    ...
    // TODO: Should audio processing be in its own thread, where the only thing this does directly is copy the data
    //  over? I think audio output is delayed until these events complete, so that could cause issues if this processing
    //  takes too long
    private void OnAudioFilterRead(float[] data, int channels)
    {
        ...
        {
            _idle = false;
            //var deltaTime = (double)(timestamp - _lastRequestTimestamp) / Stopwatch.Frequency;
    ...
    {
        // TODO: DEBUG
        Log($"Idle excess buffer popped at {InfoPanel.FormatTime(Recording.RecordTime)}; count = {count}");
